<?php
	...
	$hat1 = addslashes($_GET["hat1"]);
	$hat2 = addslashes($_GET["hat2"]);
	$hat3 = addslashes($_GET["hat3"]);
	$shirt = addslashes($_GET["shirt"]);
	$pants = addslashes($_GET["pants"]);
	$tshirt = addslashes($_GET["tshirt"]);
	$hc = addslashes($_GET["hc"]);
	$tc = addslashes($_GET["tc"]);
	$la = addslashes($_GET["la"]);
	$ll = addslashes($_GET["ll"]);
	$ra = addslashes($_GET["ra"]);
    ...
    $joinscript = [
        ...
        "UserId" => $id,
        "SuperSafeChat" => false,
        "CharacterAppearance" => "http://www.roblox.com/asset/characterfetch.ashx?hat1=$hat1&hat2=$hat2&hat3=$hat3&shirt=$shirt&pants=$pants&tshirt=$tshirt&hc=$hc&tc=$tc&la=$la&ll=$ll&ra=$ra&rl=$rl",
        "ClientTicket" => "",
        "GameId" => 3,
        "PlaceId" => 1818,
